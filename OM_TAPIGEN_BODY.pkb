CREATE OR REPLACE PACKAGE BODY om_tapigen IS
  ...
  TYPE t_tab_column_info IS TABLE OF t_column_info INDEX BY BINARY_INTEGER;

  TYPE t_unique_constraint_info IS RECORD(
     constraint_name user_constraints.constraint_name%TYPE);

  TYPE t_tab_unique_constraint_info IS TABLE OF t_unique_constraint_info INDEX BY BINARY_INTEGER;
  ...
  TYPE t_tab_unique_cons_column_info IS TABLE OF t_unique_cons_column_info INDEX BY BINARY_INTEGER;

  SUBTYPE st_substitution_key IS VARCHAR2(100 CHAR);

  SUBTYPE st_substitution_value IS VARCHAR2(32767 CHAR);

  TYPE t_tab_substitutions_array IS TABLE OF st_substitution_value INDEX BY st_substitution_key;
  ...
  CURSOR g_cur_sequence_exists IS
    SELECT sequence_name FROM user_sequences WHERE sequence_name = g_sequence_name;
  ...
  PROCEDURE util_template_replace(p_scope IN VARCHAR2 DEFAULT NULL) IS
    ...
    BEGIN
      v_match_pos := regexp_instr(g_tapi_code_blocks.template
                                 ,v_pattern
                                 ,v_start_pos);
    END get_match_pos;
  ...
  BEGIN
    ...
    LOOP
      ...
      IF v_match_pos > 0
      THEN
        v_match_len := instr(g_tapi_code_blocks.template
                            ,'#'
                            ,v_match_pos
                            ,2) - v_match_pos;
        v_match     := substr(g_tapi_code_blocks.template
                             ,v_match_pos
                             ,v_match_len + 1);
        -- (1) process text before the match
        code_append(substr(g_tapi_code_blocks.template
                          ,v_start_pos
        ...
        EXCEPTION
          WHEN no_data_found THEN
            raise_application_error(c_generator_error_number
                                   ,'FIXME: Bug - Substitution ' || v_match || ' not initialized');
        END;
      ELSE
    ...
    FOR i IN (WITH cons AS
                ...
                cols AS
                 (SELECT constraint_name
                  ...
                  WHERE table_name = p_table_name)
                SELECT column_name FROM cons JOIN cols ON cons.constraint_name = cols.constraint_name ORDER BY position)
    LOOP
      v_table_pk := v_table_pk || p_delimiter || i.column_name;
    END LOOP;
  
    RETURN ltrim(v_table_pk
                ,p_delimiter);
  ...
  BEGIN
    v_return := CASE
                  ...
                  WHEN p_data_type = 'DATE' THEN
                   ' to_char(' || p_attribute_name || q'[, 'yyyy.mm.dd hh24:mi:ss')]'
                  WHEN p_data_type LIKE 'TIMESTAMP%' THEN
                   ' to_char(' || p_attribute_name || q'[, 'yyyy.mm.dd hh24:mi:ss.ffffff')]'
                  WHEN p_data_type = 'BLOB' THEN
  ...
  -- the table API.
  -----------------------------------------------------------------------------
  FUNCTION util_get_normalized_identifier(p_identifier VARCHAR2) RETURN VARCHAR2 IS
  BEGIN
...
CREATE OR REPLACE PACKAGE #API_NAME# IS 
   ...
   *   p_enable_update_of_rows="' || util_bool_to_string(g_enable_update_of_rows) || '"
   *   p_enable_deletion_of_rows="' || util_bool_to_string(g_enable_deletion_of_rows) || '"
   *   p_enable_generic_change_log="' ||
                                   util_bool_to_string(g_enable_generic_change_log) || '"
   *   p_enable_dml_view="' || util_bool_to_string(g_enable_dml_view) || '"
...
CREATE OR REPLACE PACKAGE BODY #API_NAME# IS
  ...
  BEGIN
    ...
    THEN
      FOR i IN g_tab_unique_constraint_info.first .. g_tab_unique_constraint_info.last
      LOOP
        g_tab_substitutions_array('#PARAM_LIST_UNIQUE#') := NULL;
        g_tab_substitutions_array('#COLUMN_COMPARE_LIST_UNIQUE#') := NULL;
      
        FOR j IN g_tab_unique_cons_column_info.first .. g_tab_unique_cons_column_info.last
        LOOP
          IF g_tab_unique_cons_column_info(j).constraint_name = g_tab_unique_constraint_info(i).constraint_name
          THEN
            g_tab_unique_cons_column_info(j).parameter_name := CASE
                                                                 WHEN g_enable_parameter_prefixes THEN
                                                                  'p_' || substr(g_tab_unique_cons_column_info(j).column_name
                                                                                ,1
                                                                                ,28)
                                                                 ELSE
                                                                  g_tab_unique_cons_column_info(j).column_name
                                                               END;
            g_tab_substitutions_array('#PARAM_LIST_UNIQUE#') := g_tab_substitutions_array('#PARAM_LIST_UNIQUE#') ||
                                                                c_list_delimiter || CASE
                                                                  WHEN g_enable_parameter_prefixes THEN
                                                                   g_tab_unique_cons_column_info(j).parameter_name
                                                                  ELSE
                                                                   g_tab_unique_cons_column_info(j).parameter_name
                                                                END || ' ' || g_table_name || '."' || g_tab_unique_cons_column_info(j)
                                                               .column_name || '"%TYPE';
            g_tab_substitutions_array('#COLUMN_COMPARE_LIST_UNIQUE#') := g_tab_substitutions_array('#COLUMN_COMPARE_LIST_UNIQUE#') ||
                                                                         '         AND ' ||
                                                                         util_get_attribute_compare(p_data_type         => g_tab_unique_cons_column_info(j)
                                                                                                                           .data_type
                                                                                                   ,p_first_attribute   => '"' || g_tab_unique_cons_column_info(j)
                                                                                                                          .column_name || '"'
                                                                                                   ,p_second_attribute  => g_tab_unique_cons_column_info(j)
                                                                                                                           .parameter_name
                                                                                                   ,p_compare_operation => '=') ||
                                                                         c_crlf;
          END IF;
        ...
        g_tab_substitutions_array('#PARAM_LIST_UNIQUE#') := ltrim(g_tab_substitutions_array('#PARAM_LIST_UNIQUE#')
                                                                 ,c_list_delimiter);
        g_tab_substitutions_array('#COLUMN_COMPARE_LIST_UNIQUE#') := rtrim(ltrim(g_tab_substitutions_array('#COLUMN_COMPARE_LIST_UNIQUE#')
                                                                                ,'         AND ')
                                                                          ,c_crlf);
        g_tapi_code_blocks.template := ' 
  ...
  BEGIN
    ...
    THEN
      ...
      LOOP
        ...
        LOOP
          ...
          THEN
            g_tab_unique_cons_column_info(j).parameter_name := CASE
                                                               ...
                                                               END;
            g_tab_substitutions_array('#PARAM_LIST_UNIQUE#') := g_tab_substitutions_array('#PARAM_LIST_UNIQUE#') ||
                                                                c_list_delimiter || g_tab_unique_cons_column_info(j)
                                                               .parameter_name || ' ' || g_table_name || '."' || g_tab_unique_cons_column_info(j)
                                                               .column_name || '"%TYPE';
            g_tab_substitutions_array('#PARAM_CALLING_LIST_UNIQUE#') := g_tab_substitutions_array('#PARAM_CALLING_LIST_UNIQUE#') ||
                                                                        c_list_delimiter || g_tab_unique_cons_column_info(j)
                                                                       .parameter_name || ' => ' || g_tab_unique_cons_column_info(j)
                                                                       .parameter_name;
            g_tab_substitutions_array('#COLUMN_COMPARE_LIST_UNIQUE#') := g_tab_substitutions_array('#COLUMN_COMPARE_LIST_UNIQUE#') ||
                                                                         '         AND ' ||
                                                                         util_get_attribute_compare(p_data_type         => g_tab_unique_cons_column_info(j)
        ...
        g_tab_substitutions_array('#PARAM_CALLING_LIST_UNIQUE#') := ltrim(g_tab_substitutions_array('#PARAM_CALLING_LIST_UNIQUE#')
                                                                         ,c_list_delimiter);
  ...
  IS
  BEGIN
    DELETE FROM #TABLE_NAME# WHERE "#PK_COLUMN#" = #PK_COLUMN_PARAMETER#;' || CASE
                                     WHEN g_enable_generic_change_log THEN
                                      q'[
    create_change_log_entry( p_table     => '#TABLE_NAME#'
                           , p_column    => '#PK_COLUMN#'
  ...
  BEGIN
    ...
    LOOP
      IF (g_tab_column_info(i).column_name <> g_tab_substitutions_array('#PK_COLUMN#'))
      THEN
        g_tab_substitutions_array('#I_COLUMN_NAME#') := g_tab_column_info(i).column_name;
        g_tab_substitutions_array('#I_COLUMN_NAME_26#') := g_tab_column_info(i).column_name_26;
        g_tapi_code_blocks.template := ' 
  ...
  BEGIN
    ...
    LOOP
      ...
      THEN
        g_tab_substitutions_array('#I_COLUMN_NAME#') := g_tab_column_info(i).column_name;
        g_tab_substitutions_array('#I_COLUMN_NAME_26#') := g_tab_column_info(i).column_name_26;
        g_tab_substitutions_array('#I_PARAMETER_NAME#') := CASE
                                                             WHEN g_enable_parameter_prefixes THEN
                                                              'p_' || g_tab_column_info(i).column_name_28
                                                             ELSE
                                                              g_tab_column_info(i).column_name
                                                           END;
        g_tab_substitutions_array('#I_COLUMN_COMPARE#') := util_get_attribute_compare(p_data_type         => g_tab_column_info(i)
                                                                                                             .data_type
                                                                                     ,p_first_attribute   => 'v_row."' || g_tab_column_info(i)
                                                                                                            .column_name || '"'
                                                                                     ,p_second_attribute  => g_tab_substitutions_array('#I_PARAMETER_NAME#')
                                                                                     ,p_compare_operation => '<>');
        g_tab_substitutions_array('#I_OLD_VALUE#') := util_get_vc2_4000_operation(p_data_type      => g_tab_column_info(i)
                                                                                                      .data_type
                                                                                 ,p_attribute_name => 'v_row."' || g_tab_column_info(i)
                                                                                                     .column_name || '"');
        g_tab_substitutions_array('#I_NEW_VALUE#') := util_get_vc2_4000_operation(p_data_type      => g_tab_column_info(i)
                                                                                                      .data_type
                                                                                 ,p_attribute_name => g_tab_substitutions_array('#I_PARAMETER_NAME#'));
        g_tapi_code_blocks.template := ' 
    ...
    IF v_row."#PK_COLUMN#" IS NOT NULL THEN
      ...
      IF #I_COLUMN_COMPARE# THEN
        UPDATE #TABLE_NAME#
         ...
         WHERE "#PK_COLUMN#" = #PK_COLUMN_PARAMETER#;' || CASE
                                         WHEN g_enable_generic_change_log THEN
                                          q'[
        create_change_log_entry( p_table     => '#TABLE_NAME#'
...
BEGIN
  IF INSERTING THEN 
    ' || CASE
                                     WHEN g_enable_insertion_of_rows THEN
  ...
  ELSIF UPDATING THEN 
    ' || CASE
                                     WHEN g_enable_update_of_rows THEN
    ...
    PROCEDURE initialize IS
      ...
      BEGIN
        ...
        g_col_prefix_in_method_names := CASE
                                          WHEN g_reuse_existing_api_params AND v_api_found AND
        ...
        g_enable_insertion_of_rows := CASE
                                        WHEN g_reuse_existing_api_params AND v_api_found AND
                                             g_params_existing_api.p_enable_insertion_of_rows IS NOT NULL THEN
                                         util_string_to_bool(g_params_existing_api.p_enable_insertion_of_rows)
                                        ELSE
                                         p_enable_insertion_of_rows
                                      END;
      
        g_enable_update_of_rows := CASE
                                     WHEN g_reuse_existing_api_params AND v_api_found AND
                                          g_params_existing_api.p_enable_update_of_rows IS NOT NULL THEN
                                      util_string_to_bool(g_params_existing_api.p_enable_update_of_rows)
                                     ELSE
        ...
        g_enable_dml_view := CASE
                               WHEN g_reuse_existing_api_params AND v_api_found AND
                                    g_params_existing_api.p_enable_dml_view IS NOT NULL THEN
                                util_string_to_bool(g_params_existing_api.p_enable_dml_view)
                               ELSE
                                p_enable_dml_view
                             END;
        ...
        g_api_name := CASE
                        WHEN g_reuse_existing_api_params AND v_api_found AND g_params_existing_api.p_api_name IS NOT NULL THEN
        ...
        g_enable_getter_and_setter := CASE
                                        WHEN g_reuse_existing_api_params AND v_api_found AND
                                             g_params_existing_api.p_enable_getter_and_setter IS NOT NULL THEN
                                         util_string_to_bool(g_params_existing_api.p_enable_getter_and_setter)
                                        ELSE
                                         p_enable_getter_and_setter
                                      END;
        g_enable_parameter_prefixes := CASE
                                         WHEN g_reuse_existing_api_params AND v_api_found AND
                                              g_params_existing_api.p_enable_parameter_prefixes IS NOT NULL THEN
                                          util_string_to_bool(g_params_existing_api.p_enable_parameter_prefixes)
                                         ELSE
                                          p_enable_parameter_prefixes
                                       END;
        g_return_row_instead_of_pk := CASE
                                        WHEN g_reuse_existing_api_params AND v_api_found AND
                                             g_params_existing_api.p_return_row_instead_of_pk IS NOT NULL THEN
                                         util_string_to_bool(g_params_existing_api.p_return_row_instead_of_pk)
                                        ELSE
                                         p_return_row_instead_of_pk
                                      END;
      ...
      BEGIN
        ...
        g_tab_substitutions_array('#COLUMN_PREFIX#') := util_get_table_column_prefix(g_table_name);
      
        IF g_col_prefix_in_method_names = FALSE AND g_tab_substitutions_array('#COLUMN_PREFIX#') IS NULL
        THEN
          raise_application_error(c_generator_error_number
        ...
        g_tab_substitutions_array('#TABLE_NAME_24#') := substr(g_table_name
                                                              ,1
                                                              ,24);
        g_tab_substitutions_array('#TABLE_NAME_26#') := substr(g_table_name
                                                              ,1
                                                              ,26);
        g_tab_substitutions_array('#TABLE_NAME_28#') := substr(g_table_name
                                                              ,1
                                                              ,28);
        g_tab_substitutions_array('#PK_COLUMN#') := util_get_table_key(p_table_name => g_table_name);
        g_tab_substitutions_array('#PK_COLUMN_26#') := substr(g_tab_substitutions_array('#PK_COLUMN#')
                                                             ,1
                                                             ,26);
        g_tab_substitutions_array('#PK_COLUMN_28#') := substr(g_tab_substitutions_array('#PK_COLUMN#')
                                                             ,1
                                                             ,28);
        g_tab_substitutions_array('#PK_COLUMN_PARAMETER#') := CASE
                                                                WHEN g_enable_parameter_prefixes THEN
                                                                 'p_' || g_tab_substitutions_array('#PK_COLUMN_28#')
                                                                ELSE
                                                                 g_tab_substitutions_array('#PK_COLUMN#')
                                                              END;
        g_tab_substitutions_array('#RETURN_TYPE#') := CASE
                                                        WHEN g_return_row_instead_of_pk THEN
                                                         g_table_name || '%ROWTYPE'
                                                        ELSE
                                                         g_table_name || '."' || g_tab_substitutions_array('#PK_COLUMN#') ||
                                                         '"%TYPE'
                                                      END;
      
        -- must be after g_tab_substitutions_array('#TABLE_NAME#') and #PK_COLUMN# and #COLUMN_PREFIX# are defined
        ...
        g_tab_substitutions_array('#GENERATOR_VERSION#') := c_generator_version;
        g_tab_substitutions_array('#GENERATOR_ACTION#') := p_generator_action;
        g_tab_substitutions_array('#GENERATED_AT#') := to_char(SYSDATE
      ...
      BEGIN
        ...
        THEN
          ...
          LOOP
            -- check constraint
            SELECT COUNT(*) INTO v_count FROM user_objects WHERE object_name = 'GENERIC_CHANGE_LOG_PK';
            ...
            -- check sequence
            SELECT COUNT(*) INTO v_count FROM user_objects WHERE object_name = 'GENERIC_CHANGE_LOG_SEQ';
            ...
            -- check index
            SELECT COUNT(*) INTO v_count FROM user_objects WHERE object_name = 'GENERIC_CHANGE_LOG_IDX';
...
comment on column generic_change_log.gcl_timestamp is 'The time when the change occured']';
      ...
      BEGIN
        ...
        dbms_lob.createtemporary(lob_loc => g_tapi_code_blocks.dml_view
                                ,cache   => FALSE);
        dbms_lob.createtemporary(lob_loc => g_tapi_code_blocks.dml_view_trigger
                                ,cache   => FALSE);
      ...
      PROCEDURE set_substitutions_collect_base IS
        ...
        BEGIN
          ...
          g_tab_substitutions_array('#MAP_ROWTYPE_COL_TO_PARAM_W_PK#') := NULL;
          g_tab_substitutions_array('#PARAM_DEFINITION_W_PK#') := NULL;
          g_tab_substitutions_array('#PARAM_DEFINITION_W_PK#') := NULL;
          g_tab_substitutions_array('#PARAM_IO_DEFINITION_W_PK#') := NULL;
          g_tab_substitutions_array('#PARAM_LIST_WO_PK#') := NULL;
          g_tab_substitutions_array('#SET_PARAM_TO_COLUMN_WO_PK#') := NULL;
          g_tab_substitutions_array('#SET_ROWTYPE_COL_TO_PARAM_WO_PK#') := NULL;
          g_tab_substitutions_array('#COLUMN_COMPARE_LIST_WO_PK#') := NULL;
          g_tab_substitutions_array('#PARAM_CALLING_LIST_UNIQUE#') := NULL;
        ...
        BEGIN
          ...
          FETCH g_cur_columns BULK COLLECT
            INTO g_tab_column_info LIMIT c_bulk_collect_limit;
        ...
        BEGIN
          ...
          FETCH g_cur_unique_constraints BULK COLLECT
            INTO g_tab_unique_constraint_info LIMIT c_bulk_collect_limit;
        ...
        BEGIN
          ...
          FETCH g_cur_unique_cons_columns BULK COLLECT
            INTO g_tab_unique_cons_column_info LIMIT c_bulk_collect_limit;
        ...
        BEGIN
          g_tab_column_info(i).column_name_26 := CASE
                                                   WHEN g_col_prefix_in_method_names THEN
                                                    substr(g_tab_column_info(i).column_name
                                                          ,1
                                                          ,26)
                                                   ELSE
                                                    substr(g_tab_column_info(i).column_name
                                                          ,length(g_tab_substitutions_array('#COLUMN_PREFIX#')) + 2
                                                          ,26)
                                                 END;
        
          g_tab_column_info(i).column_name_28 := substr(g_tab_column_info(i).column_name
                                                       ,1
                                                       ,28);
        
          -- normalize column names by replacing
          -- all special characters with "_"
          g_tab_column_info(i).column_name_26 := util_get_normalized_identifier(p_identifier => g_tab_column_info(i)
                                                                                                .column_name_26);
        
          g_tab_column_info(i).column_name_28 := util_get_normalized_identifier(p_identifier => g_tab_column_info(i)
                                                                                                .column_name_28);
        ...
        BEGIN
          ...
          , col3
          , ... */
          g_tab_substitutions_array('#COLUMN_LIST_W_PK#') := g_tab_substitutions_array('#COLUMN_LIST_W_PK#') ||
                                                             c_list_delimiter || '"' || g_tab_column_info(i)
                                                            .column_name || '"';
        ...
        BEGIN
          ...
          , p_col3 => :new.col3
          , ... */
          g_tab_substitutions_array('#MAP_NEW_TO_PARAM_W_PK#') := g_tab_substitutions_array('#MAP_NEW_TO_PARAM_W_PK#') ||
                                                                  c_list_delimiter || CASE
                                                                    WHEN g_enable_parameter_prefixes THEN
                                                                     'p_' || g_tab_column_info(i).column_name_28
                                                                    ELSE
                                                                     g_tab_column_info(i).column_name
                                                                  END || ' => :new."' || g_tab_column_info(i).column_name || '"';
        ...
        BEGIN
          ...
          , p_col3 => p_col3
          , ... */
          g_tab_substitutions_array('#MAP_PARAM_TO_PARAM_W_PK#') := g_tab_substitutions_array('#MAP_PARAM_TO_PARAM_W_PK#') ||
                                                                    c_list_delimiter || CASE
                                                                      WHEN g_enable_parameter_prefixes THEN
                                                                       'p_' || g_tab_column_info(i).column_name_28
                                                                      ELSE
        ...
        BEGIN
          ...
          , p_col3 => p_row.col3
          , ... */
          g_tab_substitutions_array('#MAP_ROWTYPE_COL_TO_PARAM_W_PK#') := g_tab_substitutions_array('#MAP_ROWTYPE_COL_TO_PARAM_W_PK#') ||
                                                                          c_list_delimiter || CASE
                                                                            WHEN g_enable_parameter_prefixes THEN
                                                                             'p_' || g_tab_column_info(i).column_name_28
                                                                            ELSE
                                                                             g_tab_column_info(i).column_name
                                                                          END || ' => p_row."' || g_tab_column_info(i)
                                                                         .column_name || '"';
        ...
        BEGIN
          ...
          , ... */
          g_tab_substitutions_array('#PARAM_DEFINITION_W_PK#') := g_tab_substitutions_array('#PARAM_DEFINITION_W_PK#') ||
                                                                  c_list_delimiter || CASE
                                                                    WHEN g_enable_parameter_prefixes THEN
                                                                     'p_' || g_tab_column_info(i).column_name_28
                                                                    ELSE
                                                                     g_tab_column_info(i).column_name
                                                                  END || ' IN ' || g_table_name || '."' || g_tab_column_info(i)
                                                                 .column_name || '"%TYPE' || CASE
                                                                    WHEN (g_tab_column_info(i)
                                                                         .column_name = g_tab_substitutions_array('#PK_COLUMN#')) THEN
                                                                     ' DEFAULT NULL'
                                                                    ELSE
                                                                     NULL
                                                                  END;
        ...
        BEGIN
          ...
          , ... */
          g_tab_substitutions_array('#PARAM_IO_DEFINITION_W_PK#') := g_tab_substitutions_array('#PARAM_IO_DEFINITION_W_PK#') ||
                                                                     c_list_delimiter || CASE
                                                                       WHEN g_enable_parameter_prefixes THEN
                                                                        'p_' || g_tab_column_info(i).column_name_28
                                                                       ELSE
                                                                     ...
                                                                     END || CASE
                                                                       WHEN g_tab_column_info(i)
                                                                        .column_name =
                                                                             g_tab_substitutions_array('#PK_COLUMN#') THEN
                                                                        ' IN '
                                                                       ELSE
                                                                        ' OUT NOCOPY '
                                                                     END || g_table_name || '."' || g_tab_column_info(i)
                                                                    .column_name || '"%TYPE';
        ...
        BEGIN
          ...
          IF (g_tab_column_info(i).column_name <> g_tab_substitutions_array('#PK_COLUMN#'))
          THEN
            g_tab_substitutions_array('#PARAM_LIST_WO_PK#') := g_tab_substitutions_array('#PARAM_LIST_WO_PK#') ||
                                                               c_list_delimiter || CASE
                                                                 WHEN g_enable_parameter_prefixes THEN
                                                                  'p_' || g_tab_column_info(i).column_name_28
                                                                 ELSE
        ...
        BEGIN
          ...
          IF (g_tab_column_info(i).column_name <> g_tab_substitutions_array('#PK_COLUMN#'))
          THEN
            g_tab_substitutions_array('#SET_PARAM_TO_COLUMN_WO_PK#') := g_tab_substitutions_array('#SET_PARAM_TO_COLUMN_WO_PK#') ||
                                                                        c_list_delimiter || '"' || g_tab_column_info(i)
                                                                       .column_name || '"' || ' = ' || CASE
                                                                          WHEN g_enable_parameter_prefixes THEN
                                                                           'p_' || g_tab_column_info(i).column_name_28
        ...
        BEGIN
          ...
          IF (g_tab_column_info(i).column_name <> g_tab_substitutions_array('#PK_COLUMN#'))
          THEN
            g_tab_substitutions_array('#SET_ROWTYPE_COL_TO_PARAM_WO_PK#') := g_tab_substitutions_array('#SET_ROWTYPE_COL_TO_PARAM_WO_PK#') || CASE
                                                                               WHEN g_enable_parameter_prefixes THEN
                                                                                'p_' || g_tab_column_info(i).column_name_28
                                                                               ELSE
                                                                                g_tab_column_info(i).column_name
                                                                             END || ' := v_row."' || g_tab_column_info(i)
                                                                            .column_name || '"; ';
          END IF;
        ...
        BEGIN
          ...
          IF (g_tab_column_info(i).column_name <> g_tab_substitutions_array('#PK_COLUMN#'))
          THEN
            g_tab_substitutions_array('#COLUMN_COMPARE_LIST_WO_PK#') := g_tab_substitutions_array('#COLUMN_COMPARE_LIST_WO_PK#') || CASE
                                                                          WHEN g_enable_generic_change_log THEN
                                                                           '      IF '
                                                                          ELSE
                                                                           '      OR '
                                                                        END ||
                                                                        util_get_attribute_compare(p_data_type         => g_tab_column_info(i)
                                                                                                                          .data_type
                                                                                                  ,p_first_attribute   => 'v_row."' || g_tab_column_info(i)
                                                                                                                         .column_name || '"'
      ...
      create_change_log_entry( p_table     => ''' ||
                                                                           g_table_name || '''
                             , p_column    => ''' || g_tab_column_info(i)
                                                                          .column_name || '''
                             , p_pk_id     => v_row."' ||
                                                                           g_tab_substitutions_array('#PK_COLUMN#') || '"
                             , p_old_value => ' ||
                                                                           util_get_vc2_4000_operation(p_data_type      => g_tab_column_info(i)
                                                                                                                           .data_type
                                                                                                      ,p_attribute_name => 'v_row."' || g_tab_column_info(i)
                                                                                                                          .column_name || '"') || '
                             , p_new_value => ' ||
      ...
      END IF;' || c_crlf
        ...
        BEGIN
          ...
          g_tab_substitutions_array('#SET_PARAM_TO_COLUMN_WO_PK#') := ltrim(g_tab_substitutions_array('#SET_PARAM_TO_COLUMN_WO_PK#')
                                                                           ,c_list_delimiter);
          g_tab_substitutions_array('#COLUMN_LIST_W_PK#') := ltrim(g_tab_substitutions_array('#COLUMN_LIST_W_PK#')
                                                                  ,c_list_delimiter);
          g_tab_substitutions_array('#MAP_NEW_TO_PARAM_W_PK#') := ltrim(g_tab_substitutions_array('#MAP_NEW_TO_PARAM_W_PK#')
                                                                       ,c_list_delimiter);
          g_tab_substitutions_array('#MAP_PARAM_TO_PARAM_W_PK#') := ltrim(g_tab_substitutions_array('#MAP_PARAM_TO_PARAM_W_PK#')
                                                                         ,c_list_delimiter);
          g_tab_substitutions_array('#MAP_ROWTYPE_COL_TO_PARAM_W_PK#') := ltrim(g_tab_substitutions_array('#MAP_ROWTYPE_COL_TO_PARAM_W_PK#')
                                                                               ,c_list_delimiter);
          g_tab_substitutions_array('#PARAM_DEFINITION_W_PK#') := ltrim(g_tab_substitutions_array('#PARAM_DEFINITION_W_PK#')
                                                                       ,c_list_delimiter);
          g_tab_substitutions_array('#PARAM_IO_DEFINITION_W_PK#') := ltrim(g_tab_substitutions_array('#PARAM_IO_DEFINITION_W_PK#')
                                                                          ,c_list_delimiter);
          g_tab_substitutions_array('#PARAM_LIST_WO_PK#') := ltrim(g_tab_substitutions_array('#PARAM_LIST_WO_PK#')
                                                                  ,c_list_delimiter);
          -- this has to be enhanced
          g_tab_substitutions_array('#COLUMN_COMPARE_LIST_WO_PK#') := ltrim(ltrim(rtrim(g_tab_substitutions_array('#COLUMN_COMPARE_LIST_WO_PK#')
                                                                                       ,c_crlf)
                                                                                 ,'      IF ')
                                                                           ,'      OR ') || CASE
                                                                        WHEN g_enable_generic_change_log THEN
    ...
    BEGIN
      ...
      util_clob_append(p_clob               => g_tapi_code_blocks.api_spec
                      ,p_clob_varchar_cache => g_tapi_code_blocks.api_spec_varchar_cache
                      ,p_varchar_to_append  => NULL
                      ,p_final_call         => TRUE);
      util_clob_append(p_clob               => g_tapi_code_blocks.api_body
      ...
      THEN
        util_clob_append(p_clob               => g_tapi_code_blocks.dml_view
                        ,p_clob_varchar_cache => g_tapi_code_blocks.dml_view_varchar_cache
                        ,p_varchar_to_append  => NULL
                        ,p_final_call         => TRUE);
        util_clob_append(p_clob               => g_tapi_code_blocks.dml_view_trigger
  ...
  BEGIN
    RETURN g_tapi_code_blocks.api_spec || terminator || g_tapi_code_blocks.api_body || terminator || CASE WHEN g_enable_dml_view THEN g_tapi_code_blocks.dml_view || terminator || g_tapi_code_blocks.dml_view_trigger || terminator ELSE NULL END;
  END main_return;
  ...
  (
    p_table_name                 IN user_tables.table_name%TYPE
   ,p_reuse_existing_api_params  IN BOOLEAN DEFAULT om_tapigen.c_reuse_existing_api_params
   , -- if true, the following params are ignored, if API package are already existing and params are extractable from spec
    p_col_prefix_in_method_names IN BOOLEAN DEFAULT om_tapigen.c_col_prefix_in_method_names
   ,p_enable_insertion_of_rows   IN BOOLEAN DEFAULT om_tapigen.c_enable_insertion_of_rows
   ,p_enable_update_of_rows      IN BOOLEAN DEFAULT om_tapigen.c_enable_update_of_rows
   ,p_enable_deletion_of_rows    IN BOOLEAN DEFAULT om_tapigen.c_enable_deletion_of_rows
  ...
  BEGIN
    ...
    LOOP
      ...
      -----------------------------------------------------------------------
      v_row.p_reuse_existing_api_params := coalesce(v_row.p_reuse_existing_api_params
                                                   ,util_bool_to_string(c_reuse_existing_api_params));
    
      v_row.p_col_prefix_in_method_names := coalesce(v_row.p_col_prefix_in_method_names
                                                    ,util_bool_to_string(c_col_prefix_in_method_names));
    
      v_row.p_enable_insertion_of_rows := coalesce(v_row.p_enable_insertion_of_rows
                                                  ,util_bool_to_string(c_enable_insertion_of_rows));
    
      v_row.p_enable_update_of_rows := coalesce(v_row.p_enable_update_of_rows
                                               ,util_bool_to_string(c_enable_update_of_rows));
    
      v_row.p_enable_deletion_of_rows := coalesce(v_row.p_enable_deletion_of_rows
                                                 ,util_bool_to_string(c_enable_deletion_of_rows));
    
      v_row.p_enable_generic_change_log := coalesce(v_row.p_enable_generic_change_log
                                                   ,util_bool_to_string(c_enable_generic_change_log));
    
      v_row.p_enable_dml_view := coalesce(v_row.p_enable_dml_view
                                         ,util_bool_to_string(c_enable_dml_view));
    
      v_row.p_enable_getter_and_setter := coalesce(v_row.p_enable_getter_and_setter
                                                  ,util_bool_to_string(c_enable_getter_and_setter));
    
      v_row.p_enable_parameter_prefixes := coalesce(v_row.p_enable_parameter_prefixes
                                                   ,util_bool_to_string(c_enable_parameter_prefixes));
    
      v_row.p_return_row_instead_of_pk := coalesce(v_row.p_return_row_instead_of_pk
