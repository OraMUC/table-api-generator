CREATE OR REPLACE PACKAGE BODY om_tapigen_oddgen_wrapper IS
  ...
  c_col_prefix_in_method_names CONSTANT param_type := 'Keep column prefix in method names';
  ...
  c_sequence_name              CONSTANT param_type := 'Sequence name (example: #TABLE_NAME_26#_SEQ)';
  --------------------------------------------------------------------------------
  FUNCTION get_name RETURN VARCHAR2 IS
  BEGIN
    RETURN 'github.com/OraMUC/table-api-generator (' || om_tapigen.c_generator_version || ' read the docs ;-)';
  END get_name;
  --------------------------------------------------------------------------------
